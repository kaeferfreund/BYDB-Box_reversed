...
for x in range(2000000):
    ...
    yield_today = np.int32(np.left_shift(
        np.int32(response.registers[0]), 16) | np.int32(response.registers[1]))/100.0
    if(yield_today != yield_today_):
        mqtt_client.publish("huawei/yield_today", payload=yield_today)
        yield_today_ = yield_today

    response = client.read_holding_registers(address=32106, count=2, unit=1)
    yield_total = np.int32(np.left_shift(
        np.int32(response.registers[0]), 16) | np.int32(response.registers[1]))/100.0
    if(yield_total != yield_total_):
        mqtt_client.publish("huawei/yield_total", payload=yield_total)
        yield_total_ = yield_total

    response = client.read_holding_registers(address=32078, count=2, unit=1)
    today_peak_power = np.int32(np.left_shift(
        np.int16(response.registers[0]), 16) | np.int32(response.registers[1]))/1000.0
    if(today_peak_power != today_peak_power_):
        mqtt_client.publish("huawei/today_peak_power",
                            payload=today_peak_power)
        today_peak_power_ = today_peak_power

    response = client.read_holding_registers(address=32080, count=2, unit=1)
    active_power = np.int32(np.left_shift(
        np.int16(response.registers[0]), 16) | np.int32(response.registers[1]))
    if(active_power != active_power_):
        mqtt_client.publish("huawei/active_power",
                            payload=float(active_power)/1000.0)
        active_power_ = active_power

    response = client.read_holding_registers(address=32072, count=2, unit=1)
    ac_c1 = np.int32(np.left_shift(np.int16(response.registers[0]), 16) | np.int32(
        response.registers[1]))/1000.0
    if(ac_c1 != ac_c1_):
        mqtt_client.publish("huawei/ac_c1", payload=ac_c1)
        ac_c1_ = ac_c1

    response = client.read_holding_registers(address=32074, count=2, unit=1)
    ac_c2 = np.int32(np.left_shift(np.int16(response.registers[0]), 16) | np.int32(
        response.registers[1]))/1000.0
    if(ac_c2 != ac_c2_):
    ...
    ac_c3 = np.int32(np.left_shift(np.int16(response.registers[0]), 16) | np.int32(
        response.registers[1]))/1000.0
    if(ac_c3 != ac_c3_):
